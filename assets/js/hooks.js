...
Hooks.Clipboard = {
  mounted() {
    this.el.addEventListener("click", () => {
      const contentToCopy = this.el.dataset.content || this.el.textContent.trim()
      ...
      navigator.clipboard.writeText(contentToCopy)
        .then(() => {
          ...
          if (originalIcon) {
            ...
            originalIcon.className = originalClasses.replace("text-yellow-400/70", "text-green-500")
            originalIcon.className = originalIcon.className.replace("group-hover:text-yellow-300", "group-hover:text-green-400")
            ...
            // after that, revert
            setTimeout(() => {
              originalIcon.className = originalClasses.replace("text-green-500", "text-yellow-400/70")
              originalIcon.className = originalIcon.className.replace("group-hover:text-green-400", "group-hover:text-yellow-300")
